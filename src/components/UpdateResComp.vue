...
export default defineComponent({
    ...
    async updateRes() {
      try {
        const result = await axios.put(
          `http://localhost:8000/api/update-res/${this.$route.params.id}`,
          {
  ...
  async mounted() {
    ...
    const result = await axios.get(
      "http://localhost:8000/api/show-update-res/" + this.$route.params.id
    );
    console.log(result), (this.resturant = result.data);
