    ...
    <div>
      <table align="center">
        ...
        <tr v-for="item in resturant" :key="item.id">
          ...
          <td>
            <router-link :to="'/updatePerm/' + item.id" id="home_update_btn"
              >Update</router-link
            >
...
export default defineComponent({
  ...
  methods: {
    async addRole() {
      const result = await axios.post("http://localhost:8000/api/post-perm", {
        permission: this.resturant.permission,
      });
    ...
    async loadData() {
      ...
      let result = await axios.get("http://localhost:8000/api/show-perm");
    ...
    async deleteRes(id) {
      ...
      let result = await axios.delete(
        `http://localhost:8000/api/dell-perm/${id}`
      );
